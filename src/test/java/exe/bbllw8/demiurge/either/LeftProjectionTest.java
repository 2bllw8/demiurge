...
public class LeftProjectionTest {
    ...
    public void forAll() {
        Assert.assertTrue("The forAll function should return true if the predicate is satisfied",
                new Left<>(12).<Integer>upcast().left().forAll(x -> x > 10));
        Assert.assertFalse(
                "The forAll function should return false if the predicate is not satisfied",
                new Left<>(7).<Integer>upcast().left().forAll(x -> x > 10));
        Assert.assertTrue(
                "The forAll function should return true if the projection is from a Right",
                new Right<>(7).<Integer>upcast().left().forAll(x -> x > 10));
    }
    ...
    public void map() {
        ...
        Assert.assertEquals(
                ...
                new Right<>(12),
                new Right<>(12).<Integer>upcast().left().map(x -> x + 2));
    }
